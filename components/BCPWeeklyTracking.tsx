...
interface BCPWeeklyTrackingProps {
  data: SheetDataRow[];
}

const TARGET_TYPE_TEXT =
  "ข้อมูลหลังจากการดำเนินงานตามแผน BCP : กรอกข้อมูลจริงจากการดำเนินงานตามแผน BCP ตั้งแต่วันที่ 30 มี.ค. 2569 เป็นต้นไป โดยใช้ผลรวมรายสัปดาห์ (7 วันที่ผ่านมา) / กรอกข้อมูลทุกวันจันทร์ของสัปดาห์ (หากวันจันทร์ใดตรงกับวันหยุด ให้กรอกข้อมูลในวันทำการถัดไป)";
const TARGET_PERIOD_TEXT = "สถิติระหว่างวันที่ 6 - 12 เม.ย. 2569";
const COL_B_INDEX = 1;
const COL_C_INDEX = 2;
...
export default function BCPWeeklyTracking({ data }: BCPWeeklyTrackingProps) {
  ...
  const responsesByBranch = useMemo(() => {
    ...
    data.forEach((row) => {
      ...
      const periodText = rowValues[COL_C_INDEX] || "";
      const postalCode = rowValues[COL_EF_INDEX] || "";

      if (
        typeText.trim() === TARGET_TYPE_TEXT.trim() &&
        periodText.includes(TARGET_PERIOD_TEXT.replace("สถิติ", "")) &&
        postalCode !== ""
      ) {
